class PostOffice:
    ...
    def read_inbox(self, username=None, N=None):
      ...
      if username not in self.boxes or username == None or N == None:
         return [message for message_list in self.boxes.values() for message in message_list] # doesn't change unread status
    ...
    def search_inbox(self, username, substr):
       return [message for message in self.boxes[username] if message['title'].find(substr) != -1  or message['body'].find(substr) != -1]
